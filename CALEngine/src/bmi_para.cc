...
vector<int> BMI::perform_training_iteration(){
...

    {
        ...
        for(pair<int, int> training: training_cache){
            ...
            for(int i = 0; ; i++){
                ...
                if(scorer->doc_ids_inv_map.find(para_id) == scorer->doc_ids_inv_map.end())
                    break;
                else
                    finished_judgments_para.insert(scorer->doc_ids_inv_map[para_id]);
            }
        }
    ...
    scorer_para->rescore_documents(weights, num_threads, judgments_per_iteration+(async_mode?extra_judgment_docs:0), finished_judgments, results);
